...
function extractCandidateUrls(content: string) {
  ...
  for (const match of relativeMatches) {
    candidates.add(normalizeUrl(`https://www.icarros.com.br${match}`))
  }
  ...
  return Array.from(candidates).filter((url) => {
    try {
      ...
      return parsed.hostname.includes('icarros.com.br') && (path.includes('/comprar/') || path.includes('/veiculo/'))
    } catch {
      return false
    }
  })
...
async function fetchSearchDocument(searchUrl: string) {
  ...
  if (!fallbackResponse.ok) {
    throw new Error(`iCarros search failed with status ${fallbackResponse.status}`)
  }

  return fallbackResponse.text()
}

async function runICarrosSearch(params: SearchParams) {
  const query = buildQuery(params)
  if (!query) return []
  if (params.vehicleType === 'MOTORCYCLE') return []

  const searchUrl = `https://www.icarros.com.br/ache/lista.jsp?palavra=${encodeURIComponent(query)}`
...
async function healthCheck(): Promise<ConnectorHealthCheckResult> {
  try {
    await runICarrosSearch({ query: 'gol', vehicleType: 'CAR', limit: 1 })
    return { ok: true }
  } catch (error) {
    return {
      ok: false,
      details: error instanceof Error ? error.message : 'iCarros healthcheck failed',
    }
  }
}

export const icarrosConnector: SourceConnector = {
  source: ICARROS_SOURCE,
  ...
  async search(params: SearchParams) {
    try {
      return await runICarrosSearch(params)
    } catch (error) {
      console.warn('[icarros] busca indisponivel, sem resultados nesta rodada:', error instanceof Error ? error.message : error)
      return []
    }
  },
  healthCheck,
