...
function extractCandidateUrls(content: string) {
  ...
  for (const match of relativeMatches) {
    candidates.add(normalizeUrl(`https://www.kavak.com${match}`))
  }
  ...
  return Array.from(candidates).filter((url) => {
    try {
      ...
      return parsed.hostname.includes('kavak.com') && (path.includes('/comprar/') || /\b\d{5,}\b/.test(path))
    } catch {
      return false
    }
  })
...
function titleFromUrl(url: string) {
  try {
    ...
    const listingSlug = parts.slice(1, 5).join(' ')
    return listingSlug
      .replace(/-/g, ' ')
      .replace(/\s+/g, ' ')
      .trim()
...
async function fetchSearchDocument(searchUrl: string) {
  ...
  if (!fallbackResponse.ok) {
    throw new Error(`Kavak search failed with status ${fallbackResponse.status}`)
  }

  return fallbackResponse.text()
}

async function runKavakSearch(params: SearchParams) {
  const query = buildQuery(params)
  if (!query) return []
  if (params.vehicleType === 'MOTORCYCLE') return []

  const searchUrl = `https://www.kavak.com/br/seminovos/${encodeURIComponent(query)}`
...
async function healthCheck(): Promise<ConnectorHealthCheckResult> {
  try {
    await runKavakSearch({ query: 'onix', vehicleType: 'CAR', limit: 1 })
    return { ok: true }
  } catch (error) {
    return {
      ok: false,
      details: error instanceof Error ? error.message : 'Kavak healthcheck failed',
    }
  }
}

export const kavakConnector: SourceConnector = {
  source: KAVAK_SOURCE,
  ...
  async search(params: SearchParams) {
    try {
      return await runKavakSearch(params)
    } catch (error) {
      console.warn('[kavak] busca indisponivel, sem resultados nesta rodada:', error instanceof Error ? error.message : error)
      return []
    }
  },
  healthCheck,
